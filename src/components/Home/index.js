...
const initialState = {
  displayImg: imagesList[0].imageUrl,
...
class Home extends Component {
  ...
  componentDidMount() {
    this.setTimer = setInterval(this.timeOut, 1000)
  }
  ...
  tabsChanging = tabId => {
    ...
    this.setState({tabIds: tabId})
  }

  getFilteredItems = () => {
    const {tabIds} = this.state

    const results = imagesList.filter(
      eachResult => eachResult.category === tabIds,
    )
  ...
  onChangeImage = id => {
    const {imgId, clickedImagesList, isRunning} = this.state
    if (imgId === id && isRunning) {
      const clickedList = [...clickedImagesList, id]
      this.setState({
        clickedImagesList: clickedList,
      })

      const randomImage = Math.floor(Math.random() * imagesList.length)
      this.setState({
        displayImg: imagesList[`${randomImage}`].imageUrl,
        imgId: imagesList[`${randomImage}`].id,
      })
    } else {
  ...
  render() {
    const {displayImg, time, clickedImagesList, isRunning} = this.state
    const totalScore = clickedImagesList.length
        ...
        <nav className="nav-container">
          ...
          <ul className="nav-items-container">
            <li className="score-container">
              <p className="game-score-heading">Score: </p>
              <p className="game-score">{totalScore}</p>
            </li>
            <li className="timer-container">
              ...
              />
              <p className="game-score"> {time} sec</p>
            </li>
          </ul>
        </nav>
        {isRunning ? (
          <div className="bg-game-container">
            <img src={displayImg} alt="match" className="display-image" />
          ...
          <div className="bg-game-container">
            <WinCard score={totalScore} startGame={this.startGame} />
          </div>
        )}
      </>
    )
